...
@Component({
  selector: "accordion-trigger-view",
  standalone: true,
  imports: [CommonModule],
  template: `<button type="button" #elRef0 #_root>
    <ng-content></ng-content>
  </button> `,
  changeDetection: ChangeDetectionStrategy.OnPush,
  styles: `:host { display: contents; }`,
})
export class AccordionTriggerView implements AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit() {
    if (typeof window !== "undefined") {
      ...
      this.enableAttributePassing(element, "accordion-trigger-view");
      this.setAttributes(this.elRef0()?.nativeElement, this.attributes());
    }
  }
