...
test('Angular Storybook docs and default story load successfully', async ({ page }) => {
  ...
  await page.goto('/?path=/docs/angular-accordion--overview');

  const docsFrame = page.frameLocator('#storybook-preview-iframe');

  await expect(docsFrame.getByRole('heading', { name: 'Angular Accordion' })).toBeVisible();
  await expect(
    docsFrame.getByText('Accessible disclosure content applied directly to native Angular host elements.'),
  ).toBeVisible();
  ...
  const root = frame.locator('#storybook-angular-default');
  ...
  const deliveryPanel = frame.locator('#storybook-angular-default-timeline-panel');
  const handoffPanel = frame.locator('#storybook-angular-default-handoff-panel');
